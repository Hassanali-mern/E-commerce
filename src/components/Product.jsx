...
const Product = memo(({ product }) => {
  ...
  return (
    <>
      {product ?
        <div className='flex flex-col items-center grow sm:grow-0  gap-3 h-[400px] w-[310px] product-div '>
          <div className='h-[75%] w-full lg:relative flex flex-col items-center justify-center bg-neutral-900 overflow-hidden'>
            ...
            <button onClick={() => { addToCart(product) }} className='h-[45px] hidden lg:block w-full add-btn  lg:opacity-0 lg:invisible bg-gray-50 lg:absolute right-0 bottom-0 text-[1em] font-bold [transition:all_0.5s_ease-in-out] cursor-pointer'>ADD TO CART</button>
          </div>
          <Link to={`/products/${product?.id}`} className='txt-div w-full'>
            <h2 className='w-full text-gray-50 text-[1.4em] font-semibold text-center product-title overflow-hidden text-ellipsis [display:-webkit-box] [-webkit-line-clamp:7] [-webkit-box-orient:vertical] ' >{product?.title}</h2>
