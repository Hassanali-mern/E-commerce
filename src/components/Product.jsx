    ...
    <>
      {product ?
        <div className='flex flex-col items-center  gap-3 h-[400px] w-[310px] product-div '>
          ...
          {true ? <button onClick={() => { setCart([...cart, 'abc']) }} className='h-[40px] w-[120px] p-[2px] add-btn lg:hidden  bg-gray-50  text-[1em] font-bold  cursor-pointer rounded-[5px] '>ADD TO CART</button> : null}
        </div>
        :
        <div className='flex flex-col items-center  gap-3 h-[400px] w-[310px] product-div '>
          <div className='h-[75%] w-full lg:relative flex flex-col items-center justify-center bg-neutral-900 overflow-hidden'>
          </div>
          <div to={`/products/${product?.id}`} className='txt-div w-full'>
