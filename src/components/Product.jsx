    ...
    <>
      {product ?
        <div className='flex flex-col items-center grow sm:grow-0  gap-3 md:h-[400px] h-[450px] w-[310px] product-div '>
          <div className='h-[60%] md:h-[75%] w-full lg:relative flex flex-col items-center justify-center bg-neutral-900 overflow-hidden'>
            <Link to={`/products/${product?.id}`} className='h-[80%] product-image '>
              <img
                className='h-full w-auto '
                src={product?.image} alt="product-image" />
            </Link>
            <button onClick={() => { addToCart(product.id) }} className='h-[45px] text-black hidden lg:block w-full add-btn  lg:opacity-0 lg:invisible bg-gray-50 lg:absolute right-0 bottom-0 text-[1em] font-bold [transition:all_0.5s_ease-in-out] cursor-pointer'>ADD TO CART</button>
          </div>
          ...
          <button onClick={() => { addToCart(product.id) }} className='h-[45px] w-[150px] text-black p-[2px] add-btn lg:hidden  bg-gray-50  text-[1em] font-bold  cursor-pointer rounded-[5px] '>ADD TO CART</button>
        </div>
        :
